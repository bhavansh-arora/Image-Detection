...
/**
 * Helper class that provides common initialization methods for the OpenCV library.
 * Works in static initialization mode (bundled .so libraries).
 */
public class OpenCVLoader {
    ...
    /**
     * Initializes the OpenCV static library.
     *
    ...
    public static boolean initDebug() {
        try {
            ...
            Log.i(TAG, "✅ OpenCV library loaded successfully (static mode)");
            return true;
    ...
    /**
     * CUDA is not supported in static mode.
     */
    public static boolean initDebug(boolean initCuda) {
        Log.w(TAG, "CUDA init requested but not supported in static mode.");
        return initDebug();
    }

    /**
     * Async initialization is not supported in static mode.
     */
    public static boolean initAsync(String version, android.content.Context appContext,
                                    LoaderCallbackInterface callback) {
        Log.w(TAG, "Async initialization not supported in static mode; calling initDebug().");
        boolean result = initDebug();
        if (callback != null) {
            if (result) callback.onManagerConnected(LoaderCallbackInterface.SUCCESS);
